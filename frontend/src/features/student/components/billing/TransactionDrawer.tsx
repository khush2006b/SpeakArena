...
export function TransactionDrawer() {
  ...
  const isRefunded =
    ...
    (t as any).refund_status === "processed" ||
    (t as any).refund_status === "refunded";

  const StatusIcon = ({ status }: { status: string }) => {
    if (isRefunded) return <RefreshCcw size={40} style={{ color: '#a855f7', marginBottom: '16px' }} />;
    switch (status.toLowerCase()) {
      case "success":
      case "captured":
        return <CheckCircle2 size={40} style={{ color: '#10b981', marginBottom: '16px' }} />;
      case "failed":
        return <AlertCircle size={40} style={{ color: '#ef4444', marginBottom: '16px' }} />;
      case "created":
        return <AlertCircle size={40} style={{ color: '#71717a', marginBottom: '16px' }} />;
      default:
        return <Clock size={40} style={{ color: '#f59e0b', marginBottom: '16px' }} />;
    }
  };

  const amountDisplay = Number(t.amount || 0).toLocaleString("en-IN");
  ...
  const courseTitle = (t as any).course_title || (t as any).courseTitle || course?.title || t.courseId;
        ...
        <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.04)', background: 'rgba(255,255,255,0.02)', flexShrink: 0, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <SheetHeader style={{ textAlign: 'center', width: '100%' }}>
            ...
            <SheetDescription style={{ fontSize: '0.875rem', fontWeight: 500, marginTop: '4px', textTransform: 'capitalize', color: '#9ca3af', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              Payment {displayStatus} &bull; {dateVal ? format(parseISO(dateVal), "MMM d, yyyy") : "Recently"}
            </SheetDescription>
          </SheetHeader>
        </div>
            ...
            <div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                  ...
                  <span style={{ fontWeight: 500, color: '#fff', textAlign: 'right', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{courseTitle}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                  ...
                  <span style={{ fontWeight: 600, color: isRefunded ? '#c084fc' : rawStatus === 'captured' ? '#34d399' : '#fbbf24', textTransform: 'capitalize' }}>{displayStatus}</span>
                </div>
